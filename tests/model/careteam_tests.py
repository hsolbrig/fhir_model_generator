...
class CareTeamTests(unittest.TestCase):
    ...
    def implCareTeam1(self, inst):
        ...
        self.assertEqual(inst.participant[1].coverageTiming.repeat.boundsPeriod.end.date, FHIRDate("2013-01-01").date)
        self.assertEqual(inst.participant[1].coverageTiming.repeat.boundsPeriod.end.as_json(), "2013-01-01")
        self.assertEqual(inst.participant[1].coverageTiming.repeat.boundsPeriod.start.date, FHIRDate("2010-12-23").date)
        self.assertEqual(inst.participant[1].coverageTiming.repeat.boundsPeriod.start.as_json(), "2010-12-23")
        self.assertEqual(inst.participant[1].coverageTiming.repeat.dayOfWeek[0], "mon")
        self.assertEqual(inst.participant[1].coverageTiming.repeat.frequency, 1)
        self.assertEqual(inst.participant[1].coverageTiming.repeat.period, 1)
        self.assertEqual(inst.participant[1].coverageTiming.repeat.periodUnit, "d")
        self.assertEqual(inst.participant[1].coverageTiming.repeat.when[0], "MORN")
