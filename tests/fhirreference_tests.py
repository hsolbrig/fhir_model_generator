...
import logging
import unittest
import model.questionnaire as questionnaire
import model.medication as medication
import model.resource as resource
import model.valueset as valueset
import model.patient as patient
import model.bundle as bundle
...
class TestResourceReference(unittest.TestCase):
    ...
    def testContainedResourceDetection(self):
        with io.open(os.path.join(filedir, 'test_contained_resource.json'), 'r', encoding='utf-8') as h:
            data = json.load(h)
    ...
    def testRelativeReference(self):
        with io.open(os.path.join(filedir, 'test_relative_reference.json'), 'r', encoding='utf-8') as h:
            data = json.load(h)
        q = questionnaire.Questionnaire(data)
        self.assertIsNotNone(q, "Must instantiate Questionnaire")
        self.assertEqual('Questionnaire', q.resource_type)
    ...
    def testBundleReferences(self):
        with io.open(os.path.join(filedir, 'test_bundle.json'), 'r', encoding='utf-8') as h:
            data = json.load(h)
...
class MockServer(server.FHIRServer):
    ...
    def request_json(self, path, nosign=False):
        ...
        with io.open(os.path.join(filedir, filename), 'r', encoding='utf-8') as handle:
